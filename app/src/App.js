...
const App = () => {
	...
	const getWatched = () => {
		setLiked([])
		if (watched.length > 0) setWatched([])
		else
			axios
				.get("/watched")
				.then(({ data }) => {
			...
			<div style={{ marginLeft }}>
				...
				>
					...
					<div className="box box-row">
						...
						<Button onClick={getLiked} active={liked.length > 0 ? true : false}>
							Favoritos
						</Button>
					</div>
					{(watched || liked) && (
						<div className="box">
							{watched.map((item) => (
								<Button key={item.watched} to={item.path}>
									{item.name}
								</Button>
							))}
							{liked.map((item) => (
					...
					{continueWatching && (
						<div className="box" style={{ marginTop: "5vh" }}>
							...
							<Button to={continueWatching.path}>
								{continueWatching.name}
							</Button>
						</div>
					)}
				</div>
